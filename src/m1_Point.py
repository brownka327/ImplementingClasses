...
class Point(object):
    ...
    def move_to(self, x_new, y_new):
        self.distance += math.sqrt((x_new - self.x)**2 + (y_new - self.y)**2)
        self.x = x_new
        self.y = y_new
        self.moves_made += 1
    ...
    def get_distance_from_start(self):
        return math.sqrt((self.x - self.x_origin)**2 + (self.y - self.y_origin)**2)

    def get_distance_traveled(self):
        return self.distance

    def closer_to(self, p2, p3):
        p1_p2 = math.sqrt((self.x - p2.x)**2 + (self.y - p2.y)**2)
        p1_p3 = math.sqrt((self.x - p3.x) ** 2 + (self.y - p3.y) ** 2)
